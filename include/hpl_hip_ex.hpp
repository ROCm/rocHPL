...
struct __atomic_ref_common {
  ...
  __host__ __device__ inline void store(T val, memory_order order = memory_order_seq_cst) noexcept {
    __hip_atomic_store(_ptr, val, order, scope);
  }
  __host__ __device__ inline T load(memory_order order = memory_order_seq_cst) const noexcept {
    return __hip_atomic_load(_ptr, order, scope);
  }
  __host__ __device__ inline operator T() const noexcept {
    return load();
  }
  __host__ __device__ inline T exchange(T val, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_exchange(&_ptr, val, order, scope);
  }
  __host__ __device__ inline bool compare_exchange_weak(T& expected, T desired, memory_order success_order, memory_order failure_order) noexcept {
    return __hip_atomic_compare_exchange_weak(_ptr, &expected, desired, success_order, failure_order, scope);
  }
  __host__ __device__ inline bool compare_exchange_strong(T& expected, T desired, memory_order success_order, memory_order failure_order) noexcept  {
    return __hip_atomic_compare_exchange_strong(_ptr, &expected, desired, success_order, failure_order, scope);
  }
  __host__ __device__ inline bool compare_exchange_weak(T& expected, T desired, memory_order order = memory_order_seq_cst) noexcept {
    if (memory_order_acq_rel == order)
      return __hip_atomic_compare_exchange_weak(_ptr, &expected, desired, order, memory_order_acquire, scope);
    else if (memory_order_release == order)
      return __hip_atomic_compare_exchange_weak(_ptr, &expected, desired, order, memory_order_relaxed, scope);
    else
      return __hip_atomic_compare_exchange_weak(_ptr, &expected, desired, order, order, scope);
  }
  __host__ __device__ inline bool compare_exchange_strong(T& expected, T desired, memory_order order = memory_order_seq_cst) noexcept {
    if (memory_order_acq_rel == order)
      return __hip_atomic_compare_exchange_strong(_ptr, &expected, desired, order, memory_order_acquire, scope);
    else if (memory_order_release == order)
      return __hip_atomic_compare_exchange_strong(_ptr, &expected, desired, order, memory_order_relaxed, scope);
    else
      return __hip_atomic_compare_exchange_strong(_ptr, &expected, desired, order, order, scope);
  }
  __host__ __device__ inline void wait(T val, memory_order order = memory_order_seq_cst) const noexcept {
...
struct __atomic_ref_arithmetic {
  ...
  __host__ __device__ inline T fetch_add(const T& arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_add(_ptr, arg, order, scope);
  }

  __host__ __device__ inline T fetch_sub(const T& arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_add(_ptr, -arg, order, scope);
  }
...
struct __atomic_ref_bitwise {
  ...
  __host__ __device__ inline T fetch_max(const T& arg, memory_order order = memory_order_seq_cst) const noexcept {
    return __hip_atomic_fetch_max(_ptr, arg, order, scope);
  }

  __host__ __device__ inline T fetch_min(const T& arg, memory_order order = memory_order_seq_cst) const noexcept {
    return __hip_atomic_fetch_min(_ptr, arg, order, scope);
  }

  __host__ __device__ inline T fetch_and(T arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_and(&_ptr, arg, order, scope);
  }
  __host__ __device__ inline T fetch_or(T arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_or(&_ptr, arg, order, scope);
  }
  __host__ __device__ inline T fetch_xor(T arg, memory_order order = memory_order_seq_cst) noexcept {
...
struct __atomic_ref_pointer {
  ...
  __host__ __device__ inline T fetch_add(const ptrdiff_t arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_add(_ptr, arg, order, scope);
  }

  __host__ __device__ inline T fetch_sub(const ptrdiff_t arg, memory_order order = memory_order_seq_cst) noexcept {
    return __hip_atomic_fetch_add(_ptr, -arg, order, scope);
  }
...
__device__ inline void atomic_thread_fence(memory_order order, thread_scope scope = thread_scope_system) {
  switch (scope) {
    case thread_scope_system:
      atomic_thread_fence_system(order);
      break;
    case thread_scope_device:
      atomic_thread_fence_device(order);
      break;
...
public:
  ...
  __device__ inline void wait(arrival_token&& arrival, memory_order order = memory_order_seq_cst) const {
    gen.wait(arrival, order);
  }

  __device__ inline void arrive_and_wait(memory_order order = memory_order_seq_cst) {
