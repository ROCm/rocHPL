...
void HPL_pdgesv(HPL_T_grid* GRID, HPL_T_palg* ALGO, HPL_T_pmat* A) {
  ...
  for (int i=0;i<nb;++i) curr->timers[i]=0.;
  for (int i=0;i<nb;++i) next->timers[i]=0.;
  ...
  print_pdfact_stats(curr);
  print_bcast_stats(curr);
  print_rowgather_stats(curr, HPL_UPD_2);
  print_rowgather_stats(curr, HPL_LOOK_AHEAD);
  print_rowgather_stats(curr, HPL_UPD_1);

  print_rowexchange_stats(curr, HPL_UPD_2);
  print_rowexchange_stats(curr, HPL_LOOK_AHEAD);
  ...
  for(j = jstart; j < N; j += nb) {
    ...
    print_rowscatter_stats(curr, HPL_LOOK_AHEAD);
    print_update_stats(curr, HPL_LOOK_AHEAD);

    print_pdfact_stats(next);
    print_bcast_stats(next);
    print_rowexchange_stats(curr, HPL_UPD_1);

    print_rowscatter_stats(curr, HPL_UPD_2);
    print_update_stats(curr, HPL_UPD_2);
    print_rowgather_stats(next, HPL_UPD_2);

    print_rowscatter_stats(curr, HPL_UPD_1);
    print_update_stats(curr, HPL_UPD_1);

    print_rowgather_stats(next, HPL_LOOK_AHEAD);
    print_rowgather_stats(next, HPL_UPD_1);

    print_rowexchange_stats(next, HPL_UPD_2);
    print_rowexchange_stats(next, HPL_LOOK_AHEAD);
...
void print_pdfact_stats(HPL_T_panel* PANEL) {
  ...
  float pdfact_time_f=0.;
  if (PANEL->grid->mycol==PANEL->pcol) {
    CHECK_HIP_ERROR(hipEventElapsedTime(&pdfact_time_f,
                                    pfactStart,
                                    pfactStop));
  }
  pdfact_time = pdfact_time_f;
