...
void HPL_pdpanel_init(HPL_T_grid*  GRID,
  ...
  if(PANEL->max_lwork_size < (size_t)(lwork) * sizeof(double)) {
    ...
    if(deviceMalloc(GRID, (void**)&(PANEL->LWORK), numbytes) != HPL_SUCCESS) {
      HPL_pabort(__LINE__,
                 "HPL_pdpanel_init",
  ...
  if(PANEL->max_uwork_size < (size_t)(uwork) * sizeof(double)) {
    ...
    if(deviceMalloc(GRID, (void**)&(PANEL->UWORK), numbytes) != HPL_SUCCESS) {
      HPL_pabort(__LINE__,
                 "HPL_pdpanel_init",
  ...
  *(PANEL->DINFO) = 0.0;
  ...
  if (PANEL->loc_workspace == NULL) {
    size_t numbytes = sizeof(int) * ((mp + nb-1)/nb + 1);
    ...
    numbytes = sizeof(double) * ((mp + nb-1)/nb + 1);
    ...
    numbytes = sizeof(double) * (nb * ((mp + nb-1)/nb + 1) );
    ...
    if(deviceMalloc(GRID, (void**)&(PANEL->locks), 2* sizeof(uint32_t)) != HPL_SUCCESS) {
      HPL_pabort(__LINE__,
                 "HPL_pdpanel_init",
                 "Memory allocation failed for pdfact scratch workspace.");
    }
    ...
    if(deviceMalloc(GRID, (void**)&(PANEL->host_workspace), numbytes) != HPL_SUCCESS) {
      HPL_pabort(__LINE__,
                 "HPL_pdpanel_init",
                 "Memory allocation failed for pdfact scratch workspace.");
    }
