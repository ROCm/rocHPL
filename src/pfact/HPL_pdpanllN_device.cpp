...
__global__ void pdpanllN(const int M,
                         const int NB,
                         const int JB,
  ...
  if (block==0) {
    ...
    for (int jj = 0; jj < JB; ++jj) {
      ...
      atomic_ref<double, thread_scope_device> L1jt(L1[(JJ + jj + 2) * NB + t]);
      if (t < jj + 1 && jj + 2 < JB) {
      ...
      if (jj + 2 < JB) {
        for (int kk = 0; kk < jj; ++kk) {
          if (t > kk && t < jj + 1) {
            atomic_ref<double, thread_scope_device> L1k(L1[(JJ + kk) * NB + t]);
      ...
      if (t == JJ + jj + 2 && jj + 2 < JB) {
        for (int kk = 0; kk < jj; ++kk) {
          atomic_ref<double, thread_scope_device> L1tk(L1[t * NB + kk]);
    ...
    for (int jj = 1; jj < JB; ++jj) {
      ...
      if (t < JB && jj + 1 < JB) {
        ...
        atomic_ref<double, thread_scope_device> L1jt(L1[(JJ + jj + 1) * NB + t]);
        s_work[t] = L1jt.load(memory_order_relaxed);
      }
      ...
      atomic_ref<double, thread_scope_device> L1jj(L1[(JJ + jj) * NB + (jj - 1)]);
...
void HPL_pdpanllN(HPL_T_panel* PANEL,
  ...
  if (M>0) {
    ...
    void* params[] = {&m,
                      ...
                      &host_flag,
                      &barrier};
    CHECK_HIP_ERROR(hipLaunchCooperativeKernel(pdpanllN<BLOCKSIZE>,
                                               dim3(Nblocks),
                                               dim3(BLOCKSIZE),
                                               params,
                                               0,
