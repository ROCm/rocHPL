...
__launch_bounds__(BLOCKSIZE) __global__
  ...
  if(block == 0) {
    ...
    for(int jj = 0; jj < JB; ++jj) {
      ...
      // while pivot row is found with MPI, do some of the left-looking update
      // of L1
      if(jj + 2 < JB) {
    ...
    for(int jj = 1; jj < JB; ++jj) {
      ...
      // apply updates to next column while we're looking for pivots in the
      // current on
      if(jj + 1 < JB) {
        for(int mm = m; mm < M; mm += BLOCKSIZE * Nblocks) {
          if(ii <= mm) {
