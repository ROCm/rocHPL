...
__global__ void pdpanllT(const int M,
                         const int NB,
                         const int JB,
  ...
  if (block==0) {
    ...
    for (int jj = 0; jj < JB; ++jj) {
      ...
      atomic_block_maxloc<BLOCKSIZE>(Nblocks, max_workspace + 1, s_max, s_loc, max, loc);
      loc+=1;
      ...
      atomic_ref<double, thread_scope_device> L1jt(L1[JJ + jj + 2 + t * NB]);
      if (t < jj + 1 && jj + 2 < JB) {
        s_work[t] = L1jt.load(memory_order_relaxed);
      }
      ...
      //while pivot row is found with MPI, do some of the left-looking update of L1
      if (jj + 2 < JB) {
        for (int kk = 0; kk < jj; ++kk) {
          if (t > kk && t < jj + 1) {
            atomic_ref<double, thread_scope_device> L1k(L1[JJ + kk + t * NB]);
            s_work[t] -= L1k.load(memory_order_relaxed) * s_work[kk];
          }
          block_barrier.arrive_and_wait(memory_order_acq_rel);
        }
      }
      ...
      if (t < jj + 1 && jj + 2 < JB) {
        L1jt.store(s_work[t], memory_order_relaxed);
      }
      ...
      if (t<NB) {
        ...
        // write max row to LDS
        s_work[t] = amax;
      }

      block_barrier.arrive_and_wait(memory_order_acq_rel);

      if (t == JJ + jj + 2 && jj + 2 < JB) {
        for (int kk = 0; kk < jj; ++kk) {
          atomic_ref<double, thread_scope_device> L1tk(L1[t + kk * NB]);
          amax -= s_work[JJ + kk] * L1tk.load(memory_order_relaxed);
        }
      }
    ...
    for (int jj = 1; jj < JB; ++jj) {
      ...
      if (t < JB && jj + 1 < JB) {
        //read next column of L1
        atomic_ref<double, thread_scope_device> L1jt(L1[JJ + jj + 1 + t * NB]);
        s_work[t] = L1jt.load(memory_order_relaxed);
      }
      ...
      if (t<NB) {
        if (myrow == srcrow) { //if this rank owns the src row
          for (int mm = BLOCKSIZE * (block-1); mm < M; mm += BLOCKSIZE * Nblocks) {
            if (mm <= srcloc && srcloc < mm + BLOCKSIZE) { //and my block owns the src row
              A[srcloc + t*LDA] = d_acur.load(memory_order_relaxed); //perform the swap
            }
          }
        }
      }
      ...
      atomic_ref<double, thread_scope_device> L1jj(L1[JJ + jj + (jj - 1) * NB]);
      const double l1jj = L1jj.load(memory_order_relaxed);

      for (int mm = m; mm < M; mm += BLOCKSIZE * Nblocks) {
        if (ii <= mm) {
          //last update from current column
          const double Amn   = An[mm] / gmax;
          const double Amnp1 = An[mm + LDA] - l1jj * Amn;
      ...
      if (t<NB) {
        ...
        // apply updates from previous columns to next entry
        if (t == JJ + jj + 1  && jj + 1 < JB) {
          for (int kk = 0; kk < jj; ++kk) {
            amax -= s_work[kk] * A0[loc + kk * LDA];
          }
        }
      ...
      //apply updates to next column while we're looking for pivots in the current on
      if (jj + 1 < JB) {
        for (int mm = m; mm < M; mm += BLOCKSIZE * Nblocks) {
          if (ii <= mm) {
            // updates from previous columns
            double Amnp1 = An[mm + LDA];
            for (int kk = 0; kk < jj; ++kk) {
              Amnp1 -= s_work[kk] * A0[mm + kk * LDA];
            }
            An[mm + LDA] = Amnp1;
          }
        }
...
void HPL_pdpanllT(HPL_T_panel* PANEL,
  ...
  int Nblocks = std::min(mat->pfact_max_blocks, (M+BLOCKSIZE-1)/BLOCKSIZE + 1);
  ...
  if (M>0) {
    ...
    void* params[] = {&m,
                      ...
                      &host_flag,
                      &barrier};
    CHECK_HIP_ERROR(hipLaunchCooperativeKernel(pdpanllT<BLOCKSIZE>,
                                               dim3(Nblocks),
                                               dim3(BLOCKSIZE),
                                               params,
                                               0,
